...
if [[ $count -eq 20 ]]; then
  ...
  kubectl get ${CR} -n "${NAMESPACE}"
  exit 1
fi

ASPERA_CRD="ibmasperahstss.hsts.aspera.ibm.com"
TIMEOUT=60
count=0
DESIRED_STATE="Ready"

until [[ $(kubectl get ${ASPERA_CRD}  -n  ${NAMESPACE} -o jsonpath="{range .items[*]}{.status.phase}{end}") == ${DESIRED_STATE} ||  $count -eq ${TIMEOUT} ]]; do
  echo "Waiting for ibm-aspera ${ASPERA_CRD} to come up in ${NAMESPACE}"
  count=$((count + 1))
  sleep 60
done

if [[ $count -eq 20 ]]; then
  echo "Timed out waiting for ${ASPERA_CRD} in ${NAMESPACE}"
  kubectl get all -n "${NAMESPACE}"
  exit 1
else
  echo "Found an instances of ibm-aspera ${ASPERA_CRD} in a Running state in ${NAMESPACE}"
fi
